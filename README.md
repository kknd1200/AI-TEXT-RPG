...
- 36종 × 아기·성체 72가지 굵은 도트 크레와 공개된 208개 4프레임 애니메이션, 사육장 한 곳, 먹이·분무·청소·놀이를 제공합니다.
- 베이비 → 아성체 → 성체로 성장합니다. 새 알을 받으면 기존 크레를 보관하고 다시 선택할 수 있습니다.
- 돌봄 직후와 매분 자동 저장하며, 수동 저장과 재로그인 불러오기를 제공합니다.
- 36종의 게임용 부화 확률 합계는 100%이며 도감에서 각 확률을 확인할 수 있습니다. 실제 유전 확률이 아닙니다. 기존 부화 결과는 다시 추첨하지 않습니다.
...
성장 기록은 `cre_terrarium_saves`에 저장합니다. 계정에 연결된 이전 사육장 기록은 데이터베이스에 옮겼으며, 기존 RPG 및 이전 도감 테이블을 삭제하지 않았습니다. 이번 화면은 36종 사육장 버전입니다. 이전 타이거 모프는 원래 그림을 유지합니다.
...
2026-09-08 배포 요청에 맞춰, 완성된 그림을 사용할 수 있도록 누락 동작 처리를 추가했습니다. 전체 애니메이션 제작 완료를 의미하지 않습니다.

- 원화 104장 → 아기·성체별 4프레임 동작 208/288개, 기본 PNG 72장.
- 미완성 동작 80개는 해당 모프·성장 단계의 기본 자세를 사용합니다. 빈 아틀라스 칸을 재생하거나 다른 동작을 해당 동작으로 표시하지 않습니다.
- 별도 돌봄 효과 4장은 아직 없으므로 요청하지 않습니다. 돌봄 수치와 저장은 서버에서 정상 처리합니다.
- 도감에 공개 동작 수와 준비 중 표시가 있습니다. 모프별 최종 시각 검수도 남아 있습니다.
- `art-source/cre36/runtime-sprites.tar.gz`에 포함된 PNG 108개를 빌드 시 복원합니다.
- `npm run build`는 부분 공개 자산의 파일·해시·좌표·누락 목록을 검사합니다. `npm test`는 모든 모프·성장 단계·동작이 존재하는 프레임을 선택하는지 검사합니다.
- `npm run check:sprites`는 전체 288개 동작과 효과 4장의 완료 검사이며, 현재는 의도적으로 실패합니다. `manifest.complete`는 false를 유지합니다.
- 원화 생성 재개 절차와 누락 목록: `art-source/cre36/RESUME.md`.
