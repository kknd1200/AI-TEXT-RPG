...
### 종목이 몇 개만 나오거나 금액이 너무 작을 때

키움 요청 파라미터(거래소 구분·조회일자 구분)가 안 맞으면 일부 종목만, 아주 작은
금액으로 돌아옵니다. 어떤 조합이 맞는지 **직접 찔러보고 골라주는 명령**이 있습니다.
...
```
거래소 조회일자  종목수   외인≠0   기관≠0   최대외인(억)  상위 종목
──────────────────────────────────────────────────────────────────
KRX    미포함       312      150      149        1,520.0  삼성전자, LG에너지솔루션
NXT    미포함        13        7        0           20.5  한국전력, LG유플러스
...
6가지 조합을 실제로 호출해 **종목 수·기관 값 유무·금액 규모**를 비교하고, 가장
그럴듯한 조합을 추천합니다. 추천값을 `.env` 에 넣으면 이후로 그 조합을 씁니다.

### 값이 0으로만 나올 때

파라미터를 바꿔도 특정 투자자 열이 계속 0이면, 응답 **필드명**이 파서가 기대하는 것과
다른 경우입니다. 어느 필드가 안 맞는지 바로 확인할 수 있습니다.
...
| `krflow probe` | 키움 요청 파라미터 조합을 훑어 맞는 값 찾기 |
