...
import { Heart, Droplets, Sparkles, Utensils, Save, Pencil, Leaf, Check, RefreshCw, Egg, Flame, Dices, Plus } from 'lucide-react';
import { petFetch } from '@/lib/supabase';
import { Progress } from '@/components/ui/progress';
import { Dialog, DialogContent, DialogTitle, DialogDescription, DialogTrigger } from '@/components/ui/dialog';
import { MORPHS, morphOf, stageNames, stageOf, type Pet, type Creature, type Action } from '@/lib/pet';
...
function sprite(creature:Creature,frame=0){return creature.phase==='egg'?`/assets/morphs/egg/frame-${frame}.png`:`/assets/morphs/${creature.morphId??'flame'}/frame-${frame}.png`;}
function Odds(){return <Dialog><DialogTrigger asChild><button className="odds-link"><Dices size={15}/>부화 확률 보기</button></DialogTrigger><DialogContent className="odds-dialog"><DialogTitle>어떤 크레가 태어날까요?</DialogTitle><DialogDescription>8가지 색과 무늬의 게임용 부화 확률입니다. 실제 모프의 유전 확률과는 다릅니다.</DialogDescription><div className="odds-scroll"><table className="odds-table"><thead><tr><th>색 · 무늬</th><th>등급</th><th>확률</th></tr></thead><tbody>{MORPHS.map(m=><tr key={m.id}><td><img src={`/assets/morphs/${m.id}/frame-0.png`} alt=""/><span>{m.name}<small>{m.pattern}</small></span></td><td>{m.rarity}</td><td>{m.weight/100}%</td></tr>)}</tbody></table></div><p className="odds-note">합계 100% · 매번 독립 추첨하므로 같은 모프가 다시 태어날 수 있어요. 새로고침으로 부화 결과가 바뀌지 않아요.</p></DialogContent></Dialog>;}
export default function Game({accountName,onLogout}:{accountName:string;onLogout:()=>Promise<void>}){
 const [pet,setPet]=useState<Pet|null>(null),[revision,setRevision]=useState(0),[busy,setBusy]=useState(false),[error,setError]=useState(''),[auth,setAuth]=useState(false),[message,setMessage]=useState('어떤 크레를 만나게 될까요?'),[action,setAction]=useState('idle'),[frame,setFrame]=useState(0),[renaming,setRenaming]=useState(false),[name,setName]=useState(''),[saved,setSaved]=useState(''),[cooldown,setCooldown]=useState(false),[clock,setClock]=useState(0),[hatching,setHatching]=useState(false),[hatchFrame,setHatchFrame]=useState(0),[reveal,setReveal]=useState<Creature|null>(null);
 ...
 useEffect(()=>{if(window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;const timer=setInterval(()=>setFrame(f=>(f+1)%4),360);return ()=>clearInterval(timer);},[]);
 ...
 return <main className="shell">
  ...
  <div className="game-layout"><section className="habitat-card" aria-label="크레 사육장">
   ...
   <div className={'habitat '+action+(hatching?' hatching':'')}>
    ...
    <div className={'pet-position '+(isEgg||hatching?'egg-position':'stage-'+stage)}><img className="gecko" src={hatching?`/assets/morphs/egg/frame-${hatchFrame}.png`:isEgg?`/assets/morphs/egg/frame-${eggProgress>=100?1:0}.png`:sprite(pet,frame)} alt={isEgg||hatching?'부화 준비 중인 알':`${morph.name} ${pet.name}`}/></div><span className="pet-tag">{hatching?'두근두근':pet.name} <Heart size={11} fill="currentColor"/></span></>}
  ...
  <Dialog open={!!reveal} onOpenChange={open=>{if(!open)setReveal(null);}}><DialogContent className="hatch-result"><DialogTitle>새로운 친구가 태어났어요!</DialogTitle><DialogDescription>색과 무늬가 정해졌어요. 이제 이름을 짓고 함께 자라 보세요.</DialogDescription>{reveal&&<><img src={sprite(reveal,frame)} alt={morphOf(reveal.morphId).name}/><span className="result-rarity">{morphOf(reveal.morphId).rarity} · {morphOf(reveal.morphId).weight/100}%</span><h3>{morphOf(reveal.morphId).name}</h3><p>{morphOf(reveal.morphId).pattern}</p><button onClick={()=>{setReveal(null);setName(reveal.name);setRenaming(true);}}>이름 지어주기</button></>}</DialogContent></Dialog>
 </main>;
}
